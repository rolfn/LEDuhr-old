...
/* see: http://www.mikrocontroller.net/topic/58769#1760050 */
...
const uint8_t PROGMEM BITNO[] = {
    ...
    0xFF };            // parity
const uint8_t PROGMEM BMASK[] = { 1, 2, 4, 8, 10, 20, 40, 80 };
...
void decode_dcf77( uint8_t pulse )
{
  ...
  //printHEX(3, 10, parity);
  if( i == 0xFF ){        // test parity
    if( parity ) dcf77error = 1;
    parity = 0;
    return;
  }
...
void scan_dcf77( void )
{
  ...
  if( dcf77_pulse ){
    ...
    if( dcf77_pulse > 3 && dcf77_pulse < 9 ){
      decode_dcf77( 0 );
    }else{
